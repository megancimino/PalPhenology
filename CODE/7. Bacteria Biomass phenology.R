...
bb <- read.xlsx("Pal Datasets.xlsx", sheet = "Bacteria", skipEmptyRows = FALSE)


####### Loop through each season to pull out min/max for variable of interest #####
BB_max<-NULL

for(g in 1:length(unique(bb$Season))){
  this_yr<-unique(bb$Season)[g]
  v1<-subset(bb, bb$Season == this_yr)
  v1<-v1[order(v1$Days.Sept30),]
  ###### calculate peak
  BB_max<-rbind(BB_max, rbind(
    c("BactBiomass",  this_yr, v1$Days.Sept30[which.max(v1[,6])], v1[,6][which.max(v1[,6])])))
#  dev.new()
# plot(v1$Days.Sept30, v1[,6], type="o", main=c(g, this_yr), xlim=c(1, 200))
#abline(v= v1$Days.Sept30[which.max(v1[,6])], col="green")
}

colnames( BB_max)<-c("variable","season","day_of_max", "max_BB_value")
BB_max<-as.data.frame(BB_max)
BB_max[,2]<-as.numeric(as.character(BB_max[,2]))
BB_max[,3]<-as.numeric(as.character(BB_max[,3]))
BB_max[,4]<-as.numeric(as.character(BB_max[,4]))


#remove years with missing data 
BB_max[BB_max$season %in% c(  2008, 2018),3]<-NA
BB_max[BB_max$season %in% c( 2008, 2018),4]<-NA
#save(BB_max, file="BB_max.Rdata")
...
setwd(plot_wd)
pdf("BactBiomass_summary_phenology.pdf", height=10, width=6)
par(mfrow=c(3,1)); par(oma=c(2,2,2,0))

plot(NA,NA, col="lightgray", xlim=c(0,366), ylim=c(0,155) , xlab="", ylab="Bacteria Biomass (mmol C m^-2)", cex.lab=1.5, cex.axis=1.3,,xaxs="i",yaxs="i", xaxt="n")
...
for(q in 1:length(unique(bb$Season))){
  yr1<-subset(bb, bb$Season == unique(bb$Season)[q])
  if((yr1$Season[1] %in% c(2002,  2018))==F ){
...
zzz<-bb[order(bb$Days.Sept30),]
#remove years with incomplete data 
zzz<-zzz[-which(zzz$Season %in% c(2002, 2018)==T),]
...
points(mean(BB_max$day_of_max, na.rm=TRUE), mean(BB_max$max_BB_value, na.rm=TRUE) , cex=cex_id, pch=20, col=pt_COL)
#sd for day of peak 
segments(mean(BB_max$day_of_max, na.rm=TRUE)- sd(BB_max$day_of_max, na.rm=TRUE) ,mean(BB_max$max_BB_value, na.rm=TRUE),
         mean(BB_max$day_of_max, na.rm=TRUE)+ sd(BB_max$day_of_max, na.rm=TRUE), mean(BB_max$max_BB_value, na.rm=TRUE), col=pt_COL,lwd=lwd_id)
...
segments(mean(BB_max$day_of_max, na.rm=TRUE), mean(BB_max$max_BB_value, na.rm=TRUE)- sd(BB_max$max_BB_value, na.rm=TRUE),
         mean(BB_max$day_of_max, na.rm=TRUE), mean(BB_max$max_BB_value, na.rm=TRUE)+ sd(BB_max$max_BB_value, na.rm=TRUE),col=pt_COL, lwd=lwd_id)

legend("topright", c("2002-2018", "2002-2018 Average" ), lwd=c(2,3), col=c("gray", "black"), bty="n", cex=1.5)
