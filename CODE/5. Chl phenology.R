...
chl <- read.xlsx("Pal Datasets.xlsx", sheet = "Chl", skipEmptyRows = FALSE)


####### Loop through each season to pull out min/max for variable of interest #####
Chl_max<-NULL

for(g in 1:length(unique(chl$Season))){
this_yr<-unique(chl$Season)[g]
v1<-subset(chl, chl$Season == this_yr)
v1<-v1[order(v1$Days.Sept30),]
###determine peak
Chl_max<-rbind(Chl_max, rbind(
c("CHL",  this_yr, v1$Days.Sept30[which.max(v1[,6])], v1[,6][which.max(v1[,6])])))
#dev.new()
#plot(v1$Days.Sept30, v1[,6], type="o", main=c(g, this_yr))
#abline(v= v1$Days.Sept30[which.max(v1[,6])], col="green")
}

colnames(Chl_max)<-c("variable","season","day_of_max", "max_chl_value")
Chl_max<-as.data.frame(Chl_max)
Chl_max[,2]<-as.numeric(as.character(Chl_max[,2]))
Chl_max[,3]<-as.numeric(as.character(Chl_max[,3]))
Chl_max[,4]<-as.numeric(as.character(Chl_max[,4]))

#remove seasons with missing values that may lead to missing chl peak
Chl_max[Chl_max$season %in% c(1992, 1993, 1994, 2007, 2009),3]<-NA
Chl_max[Chl_max$season %in% c(1992, 1993, 1994, 2007, 2009),4]<-NA

###### log chl
Chl_max$max_chl_value<-log10(Chl_max$max_chl_value)
#save(Chl_max, file="Chl_max.Rdata")
...
setwd(plot_wd)
pdf("Chl_summary_phenology.pdf", height=10, width=6)

par(mfrow=c(3,1))
par(oma=c(2,2,2,0))

plot(NA,NA, col="lightgray", xlim=c(0,366), ylim=c(-1.8, 1.8) , xlab="", ylab="Log10 Chlorophyll (ug/L)", cex.lab=1.5, cex.axis=1.3,,xaxs="i",yaxs="i", xaxt="n")
...
for(q in 1:length(unique(chl$Season))){
yr1<-subset(chl, chl$Season == unique(chl$Season)[q])

if((yr1$Season[1] %in% c(1992, 1993, 1994, 2007, 2009))==F ){
yr1<-yr1[order(yr1$Days.Sept30),]
lines(yr1$Days.Sept30, log10(yr1[,6]), col="gray")
}
}
box()

#mn all years
zzz<-chl[order(chl$Days.Sept30),]
zzz<-zzz[-which(zzz$Season %in% c(1992, 1993, 1994, 2007, 2009)==T),]
zzz[,6]<-log10(zzz[,6])
...
points(mean(Chl_max$day_of_max, na.rm=TRUE), mean(Chl_max$max_chl_value, na.rm=TRUE) , cex=cex_id, pch=20, col=pt_COL)
#sd for day of peak 
segments(mean(Chl_max$day_of_max, na.rm=TRUE)- sd(Chl_max$day_of_max, na.rm=TRUE) ,mean(Chl_max$max_chl_value, na.rm=TRUE),
 mean(Chl_max$day_of_max, na.rm=TRUE)+ sd(Chl_max$day_of_max, na.rm=TRUE), mean(Chl_max$max_chl_value, na.rm=TRUE), col=pt_COL,lwd=lwd_id)
...
mean(Chl_max$day_of_max, na.rm=TRUE), mean(Chl_max$max_chl_value, na.rm=TRUE)+ sd(Chl_max$max_chl_value, na.rm=TRUE),col=pt_COL, lwd=lwd_id)
